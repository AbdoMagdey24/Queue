...
public class MMC {
    ...
    public MMC(double λ, double u, int c) {
        this.λ = λ;
        this.u = u;
        this.c = c;
    }
    ...
    public double Lq() {
        double Lq;
        Lq = (((Math.pow(r(), c + 1) / c) / (factorial(c) * Math.pow(1 - (r() / c), 2))) * (1 / reverseP0()));
        return Lq;
    }
    ...
    public double Pn(int N) {
        ...
        double pn = 0;
        if (n >= 0 && c > n) {
            pn = ((Math.pow(λ, n)) / (factorial(n) * Math.pow(u, n))) * (1 / reverseP0());
            return pn;
        } else if (n >= c) {
            pn = (Math.pow(λ, n) / (Math.pow(c, n - c) * factorial(c) * Math.pow(u, n))) * (1 / reverseP0());
            return pn;
        }
        return pn;
    }
